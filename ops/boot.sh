...
if [ ! -z "$DYNATRACE_TOKEN" ];then
  ...
  /bin/sh installer.sh --set-app-log-content-access=true --set-infra-only=true --set-host-group=DC --set-host-name=${CLUSTER_NAME}-worker 2>&1 & 
fi
su postgres -c 'exec /docker-entrypoint.sh  postgres'
